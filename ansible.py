...
zip7_src = '/root/ansible-useradd/zip7.sh'
zip7_dest = '/root/zip7.sh'
zip7_txt = '/root/compress-zip7.txt'
...
fio_src = '/root/ansible-useradd/fio.sh'
fio_dest = '/root/fio.sh'
################
...
# act memory check of playbook
play_source_zip7 = dict(
        name = "run script in hosts",
        hosts = 'tmp',
        tasks = [
            dict(action=dict(module='copy', src=zip7_src, dest=zip7_dest, mode='a+x')),
            dict(action=dict(module='command', args='sh ' + zip7_dest)),
            dict(action=dict(module='command', args='cat ' + zip7_txt), register='zip7_ret'),
            dict(action=dict(module='debug', msg='{{zip7_ret.stdout_lines}}')),
            ]
            )
...
play_source_fio = dict (
        ...
        hosts = 'tmp',
        tasks = [
            dict(action=dict(module='copy', src=fio_src, dest=fio_dest, mode='a+x')),
            dict(action=dict(module='command', args='mkdir /root/fio')),
            dict(action=dict(module='command', args='sh ' + fio_dest)),
            dict(action=dict(module='command', args='cat /root/fio.txt'), register='fio_ret'),
            dict(action=dict(module='debug', msg='{{fio_ret.stdout_lines}}')),
...
play_zip7 = Play().load(play_source_zip7, variable_manager=variable_manager, loader=loader)
...
try:
    ...
    ## FIO TEST
    p_fio = re.compile('iops=[0-9]+')

    result = tqm.run(play_fio)
    func_ret = p_fio.findall(func_ret)
    fio = ''
    if len(func_ret) >= 1 :
        fio = func_ret[0]
    else :
